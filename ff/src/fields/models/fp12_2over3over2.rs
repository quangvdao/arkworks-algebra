...
impl<P: Fp12Config> QuadExtConfig for Fp12ConfigWrapper<P> {
    ...
    fn mul_base_field_by_frob_coeff(fe: &mut Self::BaseField, power: usize) {
        // We now that the quadratic extension is generated by x such that x^2 is a quadratic nonresidue a in the sixth degree base field. The coefficient for the k^th power of x is x^(q^k - 1) = a^((q^k - 1) / 2). In the cases we consider about (https://eprint.iacr.org/2006/471.pdf and https://eprint.iacr.org/2007/429.pdf), a is always the cube of an element in Fp2, so we can simplify the expression to b^((q^k - 1) / 6) where b = a^3, so that we can always multiply by an Fp2 element for optimization.
